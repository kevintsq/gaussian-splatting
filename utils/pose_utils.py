...
class Pose:
    ...
    def __call__(self, R=None, t=None, device="cuda"):
        # construct a camera pose from the given R and/or t
        assert R is not None or t is not None
        if R is None:
            if not isinstance(t, torch.Tensor):
                t = torch.tensor(t, dtype=torch.float, device=device)
            R = torch.eye(3, dtype=torch.float, device=device).repeat(*t.shape[:-1], 1, 1)
        elif t is None:
            if not isinstance(R, torch.Tensor):
                R = torch.tensor(R, dtype=torch.float, device=device)
            t = torch.zeros(R.shape[:-1], dtype=torch.float, device=device)
        else:
            if not isinstance(R, torch.Tensor):
                R = torch.tensor(R, dtype=torch.float, device=device)
            if not isinstance(t, torch.Tensor):
...
def interpolation_linear(pos, alpha):
...
def interpolation_spline(pos, alpha):
...
def interpolation_bezier(pos, alpha):
    ...
    bezier_coeff = torch.tensor(bezier_coeff, dtype=torch.float, device="cuda")
    ...
    delta_poses_mat = torch.eye(4, dtype=torch.float, device="cuda")
    for i in range(order + 1):
