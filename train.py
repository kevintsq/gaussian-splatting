...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    if args.deblur:
        blur_blend_embedding = torch.nn.Embedding(
            len(scene.getTrainCameras()), args.blur_sample_num, device=args.device)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if args.deblur:
            blur_weight = blur_blend_embedding(
                torch.tensor(viewpoint_cam.uid, device=args.device))
        ...
        image = 0
        depth = 0
        radii = torch.zeros(len(gaussians), dtype=torch.float, device=args.device)
        viewspace_point_tensors = []
        viewspace_point_tensor_data = 0
        visibility_filter = torch.zeros(len(gaussians), dtype=torch.bool, device=args.device)
        ...
        for idx in range(args.blur_sample_num):
            ...
            render_pkg = render(viewpoint_cam, gaussians, pipe, bg, use_trained_exp=dataset.train_test_exp,
                                separate_sh=SPARSE_ADAM_AVAILABLE, interp_alpha=alpha)
        ...
        if depth_l1_weight(iteration) > 0 and viewpoint_cam.depth_reliable:
            ...
            if viewpoint_cam.alpha_mask is not None:
                Ll1depth_pure = torch.abs((depth - mono_invdepth) * alpha_mask).mean()
            else:
                Ll1depth_pure = F.l1_loss(depth, mono_invdepth)
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background, 1., SPARSE_ADAM_AVAILABLE, None, dataset.train_test_exp), dataset.train_test_exp)
...
def training_report(tb_writer, iteration, Ll1, loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, train_test_exp):
    if tb_writer:
